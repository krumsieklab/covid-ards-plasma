...
# (15) gg_gap - modified gg_gap helper function for a stacked barplot segmented picture of lipidomics data 
...
get_pathway_plot <- function(plot_mat, # plot data matrix
  ...
  plot_mat %<>% dplyr::left_join(., plot_mat %>% dplyr::group_by(label) %>%
    ...
    dplyr::rename(path_wt=n)  %>% filter(path_wt>3)
  ...
  # break axis in lipids because of large size using modified gg_gap function
  if(grepl('lipids', dataset)){
    ptmp1 <- p %>% gg_gap(
      ylim = c(0, 275), 
      segments = list(c(50, 150)),
      tick_width = 10) %>% .$`1`
    ...
    grid.arrange(ptmp1,ptmp2, ncol=2)
...
get_compiled_stats <- function(sub_set, # list of files
  ...
  stat_data <- lapply(1:length(stat_list[[1]]), FUN=function(i){
    res <- do.call(plyr::rbind.fill, list(stat_list[['plasma_metabo']][[i]] %>% 
                                            dplyr::select(name, p_value, adj_p, statistic, SUB_PATHWAY) %>%
                                            dplyr::mutate(mol_type='Metabolites'), 
                               stat_list[['plasma_lipids']][[i]] %>% 
                                 dplyr::select(name, p_value, adj_p, statistic, Class) %>%
                                 dplyr::mutate(mol_type='Lipids'), 
                               stat_list[['plasma_proteo']][[i]] %>% 
...
within_ards_barplots <- function(compiled_stats, # df of all omics results combined
  ...
  for(attribute in outcomes$outcome){
    ...
    plot_mat$L1 <- factor(plot_mat$L1, levels=c("Proteins", "Lipids", "Metabolites"))
...
get_multiomics_network <- function(datasets = c("urine_metabo", "urine_proteo"),   # datasets
  ...
  names(Ds) <- c('Metabolites', 'Lipids', 'Proteins')
  ...
  ## format network data ----
  this_mat <- bind_cols(data_mat$Proteins, data_mat$Metabolites, data_mat$Lipids)
  ...
  ggm_edges %<>% dplyr::filter(abs(pcor_val)>=min(abs(pval_mat$pcor[pval_mat$p.adj.bon<=ggm_thresh])))
  ...
  all_nodes <- c(as.matrix(mol_types$Proteins$unique_name),
                 ...
                 as.matrix(mol_types$Lipids$unique_name))
...
#    edge_list must have columns named source , target, edge_type
get_manifestation_subnetwork <-  function(ggm_edges, # network
                                 ...
                                 sig_col_suf='fdr5' # suffix of significant column
                                 ) 
{
  ...
  col_ids <- sapply(group_names, FUN=function(group_name) 
                      paste0(outcome_name, '_', group_name))%>% 
  lapply(., FUN=function(x) grep(x, names(node_attributes)))%>% 
  unlist()
  ...
  this_sig_nodes <- lapply(group_names, FUN=function(group_name) {
    col_of_interest <- sprintf('%s_%s_sig_%s',outcome_name, group_name, sig_col_suf)
    which(vertex_attr(g)[[col_of_interest]]=='sig')}) %>% 
    unlist()
